...
module.exports = async (client, message) => {
  try {
    ...
    if (!channel) return message.channel.send(new MessageEmbed().Você precisa se juntar a um canal de voz."You need to join a voice channel."));
    ...
    if (player && channel.id !== player.voiceChannel) return message.channel.send(new MessageEmbed(eu já estou tocando em outro lugar"❌ I am already playVocê pode me ouvir emetDescription(`You can listen to me in: \`${message.guild.channels.cache.get(player.VoiceChannel).name}\``));
    //if the user is not in the channel as in the db voice channel return error
    if (channel.id !== db.voicechannel) return message.channel.send(new MessageEmVocê precisa estar no ${message.tle(`You need to be in the: \`${message.guild.channels.cache.get(db.voicechannel).name}\` VoiceChannel`));
