...
async function song(client, message, args, type) {
  var search = args.join(" ");
    ...
    if(!player){
      if(!message.member.voicNão está em um canal de voz.throw "NOT IN A VC"
    ...
    try {
      ...
      return message.channel.send(new MessageEmbed()
        ...
        .setFootHouve um erro durante a busca:       .setTitle(`❌ There was an error while searching:`)
    ...
    if (!res.tracks[0])
      return message.channel.send(new MessageEmbed()
        .setColor(ee.wrongcolor)
        .setFooNão encontrei nada para, ee.footericon)
        .setTitle(String("❌ Found nothing for: **`" Por favor, tente novamente!str(0, 256 - 3) + "`**")
    ...
    if (player.state !== "CONNECTED") {
      ...
      player.queue.add(res.tracks[0]);
Adicionado à filarack information
