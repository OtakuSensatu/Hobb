...
const es = require("../../botconfig/embed.json"); //Loading all embed settings like color footertext and icon ...
  ...
  try {
    ...
    if (guild_settings.botchannel.toString() !== "") {
      ...
      if (!guild_settings.botchannel.includes(message.channel.id) && !message.member.hasPermission("ADMINISTRATOR")) {
        ...
        try { message.react("815411218658754580"); } catch { }
    ...
    if (cmd.length === 0) {
      if (matchedPrefix.includes(client.user.id))
        return message.channel.send(new Discord.MessageEmbed()
          .setColor("RANDOM")
          .setTitle(`${emoji.msg.SUCCESS} **Para ver todos os tipos de Comandos: \`${prefix}help\`**`)
        );
      return;
    }
      ...
      if (command.length == 0) {
        if (guild_settings.unkowncmdmessage) {
          message.channel.send(new Discord.MessageEmbed()
            .setColor("RANDOM")
            .setFooter("")
            .setTitle(`❌ Comando desconhecido, tente: **\`${prefix}help\`**`)
            .setDescription(`O prefixo para este server é: \`${prefix}\`\n\n Você também pode me **pigar**, para saber o Prefixo!\n\n Para saber como funciona o comando digite: \`${prefix}help [nomecomando]\`\n\n Para ver todos os tipos de configurações disponíveis \`${prefix}setup\``)
          ).then(async msg => {
            try {
